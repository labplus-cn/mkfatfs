    ...
    int imageSize)
{
    ...
    *out_fs = NULL;	//MVA
    ...
    esp_partition_t *data_partition = &s_partition; //记录分区相关信息

    // connect driver to FATFS
    BYTE pdrv = 0xFF;
    if (ff_diskio_get_drive(&pdrv) != ESP_OK) { //获取一个空驱动器号
    ...
    result = ff_diskio_register_RAM_partition(pdrv, *RAM_handle, data_partition); 
    if (result != ESP_OK) {
        ESP_LOGE(TAG, "ff_diskio_register_RAM_partition failed pdrv=%i, error - 0x(%x)", pdrv, result);
        goto fail;
    }
    FATFS *fs = NULL;
...
esp_err_t emulate_esp_vfs_fat_spiflash_unmount(const char *base_path, RAM_handle_t RAM_handle)
{
    BYTE pdrv = ff_diskio_get_pdrv_RAM(RAM_handle);
    if (pdrv == 0xff) {
    ...
    // esp_err_t err_drv = wl_unmount(RAM_handle);
    // ff_diskio_unregister_RAM_partition(pdrv, RAM_handle);
