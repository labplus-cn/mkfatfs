...
typedef struct vfs_entry_ {
    ...
    char path_prefix[ESP_VFS_PATH_MAX]; // path prefix mapped to this VFS eg: /spiflash
    size_t path_prefix_len; // micro-optimization to avoid doing extra strlen
    void* ctx;              // optional pointer which can be passed to VFS 同一文件系统（如FAT）可能对应几个分区，以此区分
    int offset;             // index of this structure in s_vfs array s_vfs[]中的位置索引。
} vfs_entry_t;
...
/* 创建一个vfs_entry_t条目
    可以用维护几个vfs_entry_t条目，使之对应不同分区，并用用同一文件系统操作，
    详见ESP_IDF文档中对vfs的介绍。
*/
esp_err_t esp_vfs_register(const char* base_path, const esp_vfs_t* vfs, void* ctx)
{
    ...
    if ((len > 0 && base_path[0] != '/') || base_path[len - 1] == '/') { //right: "/spiflash" error: "/spiflash/"" or "spiflash/"
        return ESP_ERR_INVALID_ARG;
    }
    ...
    size_t index;
    for (index = 0; index < s_vfs_count; ++index) { //if unregist, some index is empty
        if (s_vfs[index] == NULL) {
            break;
        }
    }
...
/*
    应用mount文件系统时，会调用esp_vfs_register()把根路径存到vfs_entry中，
    用esp_vfs_open()打开文件时，会利用本函数数查找对应根路径的vfs_entry。
    */
static const vfs_entry_t* get_vfs_for_path(const char* path)
{
    ...
    for (size_t i = 0; i < s_vfs_count; ++i) {
        ...
        //ESP_LOGI(TAG, "%s() check for match path=%s vfs->path_prefix=%s", __func__, path, vfs->path_prefix);
...
int esp_vfs_open(struct _idf_reent *r, const char * path, int flags, int mode)
{
    ...
    //ESP_LOGI(TAG, "%s() path_within_vfs=%s", __func__, path_within_vfs);
    int ret;
